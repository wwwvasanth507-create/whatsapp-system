...
/**
 * Internal container for persisted Kyber (ML-KEM) PreKeys.
 */
internal data class StoredKyberPreKey(
    val id: Int,
    val timestamp: Long,
    val isCurrent: Boolean,
    val recordBytes: ByteArray
) {
    fun toRecord(): KyberPreKeyRecord = KyberPreKeyRecord(recordBytes)

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (javaClass != other?.javaClass) return false
        other as StoredKyberPreKey
        if (id != other.id) return false
        if (timestamp != other.timestamp) return false
        if (isCurrent != other.isCurrent) return false
        if (!recordBytes.contentEquals(other.recordBytes)) return false
